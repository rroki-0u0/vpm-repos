...
| AudioLink / LTCGI / SSAO / 深度リム / ノーマル2 / ディゾルブ / デカール / UV スクロール等 | — | ❌ レポート |
| `_EnableEnvironmentalRim` (`_RimEnviroWidth`/`Blur`/`Intensity`) | RimLight モジュール (白×強度、ライト色=環境色が乗る) | 🔶 通常リム未使用時のみ (競合時は ❌) |
| エミッション (スロット1) / 色調整 / グリッター / 環境リフレクション (+クリアコート近似) | Emission / ColorAdjust / Glitter / EnvReflection モジュール | ✅ **jp.rroki.nontoon-extra-modules** 導入時のみ (未導入なら ❌ レポート) |
...
| `_GlitterColor`/`_GlitterFrequency`/`_GlitterSpeed`/`_GlitterBrightness`/`_GlitterUseSurfaceColor` | `glitter` モジュール | 🔶 (`_GlitterSize`→輝点割合は粗い写像) |
...
- Poiyomi Fur → NonToonFur (シェル方式は近いが `_FurVector`/`_FurSubdivision` 等への写像が必要)
